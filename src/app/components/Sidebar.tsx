...
import { BookOpenText, Home, Menu, X, Minus, Bookmark } from "lucide-react";
...
export function Sidebar() {
  ...
  // Custom genres appear at the bottom but above "Other"
  const mergedGenres = [
    ...
    <>
      ...
      <div className="p-4 md:p-6 border-b border-[#caa906]">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={closeSidebar} className="md:hidden text-[#ffffff] hover:text-white">
            <X className="w-6 h-6" />
          </button>
        </div>
      ...
      {/* Navigation */}
      <nav className="flex-1 overflow-y-auto p-3 md:p-4 sidebar-scroll">
        <Link
          to="/"
        ...
        <div className="mt-4 mb-2 px-3 md:px-4">
          <h3 className="flex items-center text-xs uppercase tracking-wider text-[#caa906] font-serif">
            Chapters
            <AddGenreDialog />
      ...
      <div className="mt-4 mb-2 p-2 md:p-4">
        ...
        >
          ...
          <Bookmark
            className={`w-4 h-4 ${favActive ? "text-[#caa906]" : "text-[#caa906]"}`}
          />
        </Link>
      </div>
    </>
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed md:static inset-y-0 left-0 z-50 w-80 max-w-[85vw] bg-gradient-to-b from-[#3c0008] to-[#300006] border-r border-[#caa906] flex flex-col transform transition-transform duration-300 ${
